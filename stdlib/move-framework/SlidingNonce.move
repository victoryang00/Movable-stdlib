...
module SlidingNonce {
    ...
    ///     ```
    /// 4.Now that the window has been shifted over so that the `seq_nonce` indexlies within the new window we
    ...
    ///     ```
    /// 4.Now that the window has been shifted over so that the `seq_nonce` indexlies within the window we
