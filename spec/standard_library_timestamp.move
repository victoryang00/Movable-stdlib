address 0x1 {
    ...
    module LibraTimestamp {
        ...
        use 0x1::Signer ;
        /// A singleton resource holding the current Unixtime in microseconds
        resource struct CurrentTimeMicroseconds {microseconds : u64 , }
        ...
        public fun set_time_has_started ( lr_account : & signer ) acquires CurrentTimeMicroseconds {
            ...
            assert (
                exists < CurrentTimeMicroseconds >( CoreAddresses::
        ...
        {
        let TimeHasStarted {} = move_from < TimeHasStarted >( CoreAddresses::
        LIBRA_ROOT_ADDRESS ()); }
        */
        ...
        public fun update_global_time (
            ...
            assert ( Signer::address_of ( account ) == CoreAddresses::
            VM_RESERVED_ADDRESS () , ENOT_VM );
            let global_timer = borrow_global_mut < CurrentTimeMicroseconds >(
        ...
        public fun is_genesis () : bool { ! exists < TimeHasStarted >( CoreAddresses::LIBRA_ROOT_ADDRESS () )}
        ...
        spec module {
            ...
            define spec_is_genesis () : bool { ! exists < TimeHasStarted >( CoreAddresses::SPEC_LIBRA_ROOT_ADDRESS () )}
            /// Specification version of the `Self::is_not_initialized function.
            define spec_is_not_initialized () : bool { ! spec_root_ctm_initialized () || spec_now_microseconds () == 0}
            /// True if the association root account has a CurrentTimeMicroseconds.
            define spec_root_ctm_initialized () : bool {
                exists < CurrentTimeMicroseconds >( CoreAddresses::
                SPEC_LIBRA_ROOT_ADDRESS () )
            }
            /// Auxiliary function to get the association 's Unixtime in microseconds.
        ...
        ensures old ( spec_root_ctm_initialized () ) == > ( old (
            spec_now_microseconds () ) <= spec_now_microseconds () );
        }
        ...
        spec fun initialize {
            /* added */ moves_to global < CurrentTimeMicroseconds >( Signer::spec_address_of ( lr_account ));
        ...
        spec fun set_time_has_started {
            /* added */ moves_to global < TimeHasStarted >( Signer::spec_address_of ( lr_account ));
            aborts_if Signer::spec_address_of ( lr_account ) != CoreAddresses::SPEC_LIBRA_ROOT_ADDRESS () ;
        ...
        spec fun update_global_time {
            /* added */ modifies global < CurrentTimeMicroseconds >(CoreAddresses::SPEC_LIBRA_ROOT_ADDRESS () );
        ...
        spec fun now_microseconds {
            /* added */ ensures result == spec_now_microseconds () ;
        ...
        spec schema TimeAccessAbortsIf {
            aborts_if ! exists < CurrentTimeMicroseconds >( CoreAddresses::
            SPEC_LIBRA_ROOT_ADDRESS () );
        } 
    }
}