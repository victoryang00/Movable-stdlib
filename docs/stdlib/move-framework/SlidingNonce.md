...
indexof the window.The window size is the size of the bitmask we can
...
```
4.Now that the window has been shifted over so that the <code>seq_nonce</code> indexlies within the new window we
...
```
4.Now that the window has been shifted over so that the <code>seq_nonce</code> indexlies within the window we
